...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, cfg: DictConfig):
        ...
        self.frame_labels = {}  # Initialize frame_labels here
        ...
        # Adding labels to the combo boxes
        self.add_labels_to_ui(cfg.labels)

        # Connect the bulk assign button to the function
        self.ui.bulkAssignButton.clicked.connect(self.handle_bulk_assign_button)

    def add_labels_to_ui(self, labels):
        self.ui.labelComboBox.addItems(labels)
        self.ui.bulkLabelComboBox.addItems(labels)

    def handle_bulk_assign_button(self):
        self.assign_bulk_label(self.framenum)

    def assign_label_to_frame(self, frame, label):
        self.frame_labels[frame] = label
        print(f"Frame {frame} labeled as {label}")

    def assign_bulk_label(self, current_frame):
        if self.ui.bulkLabelCheckBox.isChecked():
            try:
                start_frame = int(self.ui.startFrameEdit.text())
                end_frame = int(self.ui.endFrameEdit.text())
                label = self.ui.bulkLabelComboBox.currentText()
                for frame in range(start_frame, end_frame + 1):
                    self.assign_label_to_frame(frame, label)
            except ValueError:
                print("Invalid frame range")
        else:
            label = self.ui.labelComboBox.currentText()
            self.assign_label_to_frame(current_frame, label)

    def save_labels_to_csv(self, file_path):
        with open(file_path, 'w', newline='') as csvfile:
            fieldnames = ['frame', 'label']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

            writer.writeheader()
            for frame, label in sorted(self.frame_labels.items()):
    ...
    def save(self):
        ...
        frame_labels_series = pd.Series(self.frame_labels, name='frame_labels')
        df = df.join(frame_labels_series, how='outer')
    ...
    def load(self, filename):
        ...
        if 'frame_labels' in df.columns:
            self.frame_labels = df['frame_labels'].dropna().to_dict()
            df = df.drop(columns=['frame_labels'])
